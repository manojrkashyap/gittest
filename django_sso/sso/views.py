...
def _get_metadata(sso_config):
    if sso_config.metadata_file is not None:
        return {
            'local': [sso_config.metadata_file.url]
        }
    else:
        return {
            'remote': [
                {
                    "url": sso_config.metadata_url,
                },
            ]
        }


def _get_saml_client(domain,sso_config):
    acs_url = sso_config.acs
    metadata = _get_metadata(sso_config)
    ...
    if sso_config.entity_id is not None:
        saml_settings['entityid'] = sso_config.entity_id
...
def acs(r):
    path = r.path
    if path is not None:
        ssoConfig_id = path.replace("/sso/acs/","")
        sso_config = SsoConfig.objects.get(id=ssoConfig_id)

    saml_client = _get_saml_client(get_current_domain(r),sso_config)
    ...
    user_attributes = get_user_mapping_attributes(user_identity)

    target_user = None
    is_new_user = False

    try:
        if settings.SAML2_AUTH.get('TRIGGER', {}).get('GET_USER', None):
            target_user =  import_string(settings.SAML2_AUTH['TRIGGER']['GET_USER'])(user_identity)
        else:
            target_user = User.objects.get(username=user_attributes['username'])
        if settings.SAML2_AUTH.get('TRIGGER', {}).get('BEFORE_LOGIN', None):
            import_string(settings.SAML2_AUTH['TRIGGER']['BEFORE_LOGIN'])(user_identity)
    except User.DoesNotExist:
        new_user_should_be_created = settings.SAML2_AUTH.get('CREATE_USER', True)
        if new_user_should_be_created:
            target_user = _create_new_user(user_attributes['username'], user_attributes['email'],
                                           user_attributes['first_name'], user_attributes['last_name'])
...
def get_user_mapping_attributes(user_identity):
    settings_attributes = settings.SAML2_AUTH.get('ATTRIBUTES_MAP', {})
    for key in settings_attributes:
        settings_attributes[key] = user_identity[settings_attributes.get(key)][0]
    return settings_attributes
...
def default_config(r):
    file_path = settings.BASE_DIR+'/django_sso/default/config_json.json'
